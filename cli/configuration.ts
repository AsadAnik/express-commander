 ...
 */
const compiledFileExt: string = path.extname(__filename);
const configPath: string = path.join(__dirname, `./config/index${compiledFileExt}`);
...
const setConfigs = (language: string | boolean, directory: string | boolean): void => {
    try {
        ...
        if (language) {
            ...
            if (compiledFileExt?.trim().toLowerCase() === '.ts') {
                configContent = configContent.replace(/export const language = '.*';/, `export const language = '${language}';`);

            } else {
                configContent = configContent.replace(/exports.language = '.*';/, `exports.language = '${language}';`);
            }
        }
        ...
        if (directory) {
            ...
            if (compiledFileExt?.trim().toLowerCase() === '.ts') {
                configContent = configContent.replace(/export const directory = '.*';/, `export const directory = '${src}';`);

            } else {
                configContent = configContent.replace(/exports.directory = '.*';/, `exports.directory = '${src}';`);
            }
        }
...
/**
 * ---- Manual Configuration with Terminal File ----
 */
